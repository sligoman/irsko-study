<section data-redesign-section="news-grid" class="home-section pb-20">
  <div class="layout-container">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      ...
      <x-text-link href="{{ route('blog') }}">Zobrazit všechny články</x-text-link>
    </div>
      ...
      @if($posts->count())
        @foreach($posts as $post)
          <article class="rounded-[16px] bg-brand-light-gray p-6 transition duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <x-text-link href="{{ route('blog.show', $post->slug) }}" class="mt-6">Číst článek</x-text-link>
          </article>
        @endforeach
      @else
        @foreach([
          ...
          <article class="rounded-[16px] bg-brand-light-gray p-6 transition duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <x-text-link href="{{ route('blog') }}" class="mt-6">Číst článek</x-text-link>
          </article>
        @endforeach
      @endif
    </div>
